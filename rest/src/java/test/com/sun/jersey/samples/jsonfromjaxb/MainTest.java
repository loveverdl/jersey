...
public class MainTest {
	...
	public void testGetOnFlightsJSONFormat() {
		...
		Flights flights = r.path("flights").accept("application/json")
				.get(Flights.class);
		// check that there are two flight entries
		assertEquals("Expected number of initial entries not found", 2, flights
				.getFlight().size());
	}
	...
	public void testPutOnFlightsJSONFormat() {
		...
		Flights updatedFlights = r.path("flights").accept("application/json")
				.get(Flights.class);
		// check that there is only one flight entry
		assertEquals(
				"Remaining number of flight entries do not match the expected value",
	...
	public void testGetOnFlightsXMLFormat() {
		// get the initial representation
		Flights flights = r.path("flights").accept("application/xml")
				.get(Flights.class);
		// check that there are two flight entries
		assertEquals("Expected number of initial entries not found", 2, flights
				.getFlight().size());
	...
	public void testPutOnFlightsXMLFormat() {
		...
		// get the updated list out from the server:
		Flights updatedFlights = r.path("flights").accept("application/XML")
				.get(Flights.class);
		// check that there is only one flight entry
		assertEquals(
				"Remaining number of flight entries do not match the expected value",
	...
	public void testGetOnAircraftsJSONFormat() {
		GenericType<List<AircraftType>> genericType = new GenericType<List<AircraftType>>() {
		};
		// get the initial representation
		...
		assertEquals("Expected number of initial aircraft types not found", 2,
				aircraftTypes.size());
	}
